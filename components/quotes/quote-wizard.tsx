      ...
      <div className="glass-panel overflow-hidden">
        ...
        <div className="min-h-[440px] p-4 sm:p-7">
          ...
          <section aria-labelledby="confirm-step" className={step === 3 ? "" : "hidden"}>
              ...
              {!isEdit && <p className="mt-3 text-xs leading-5 text-white/48">Después de guardar podrás realizar la inspección fotográfica del vehículo desde el detalle de la cotización.</p>}
          </section>
        </div>
        ...
        <div className="flex items-center justify-between gap-3 border-t border-white/10 px-4 py-4 sm:px-7">
          ...
          {step < steps.length - 1 ? <button type="button" onClick={() => setStep((value) => Math.min(steps.length - 1, value + 1))} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-white px-5 text-sm font-bold text-[#071426] hover:opacity-90">Continuar <ArrowRight className="size-4" /></button> : <button type="submit" disabled={pending} className="inline-flex min-h-12 items-center gap-2 rounded-full bg-cyan-200 px-6 text-sm font-bold text-[#071426] hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60">{pending ? <LoaderCircle className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />}{pending ? (isEdit ? "Recalculando…" : "Calculando…") : (isEdit ? "Recalcular cotización" : "Calcular cotización")}</button>}
        </div>
      </div>
    </form>
  );
