...
export function StatusChart({data}:{data:StatusRow[]}){const total=data.reduce((sum,x)=>sum+x.value,0);return <div className="grid gap-4">{data.map((row,index)=>{const pct=total?row.value/total*100:0;const colors=["bg-cyan-300","bg-blue-400","bg-amber-300","bg-rose-300"];return <div key={row.label}><div className="mb-1.5 flex items-center justify-between gap-3 text-xs"><span className="font-semibold text-white/65">{humanize(row.label)}</span><span className="text-white/45">{row.value} · {pct.toFixed(0)}%</span></div><div className="h-2 overflow-hidden rounded-full bg-white/8"><div className={`h-full rounded-full ${colors[index%colors.length]}`} style={{width:`${pct}%`}}/></div></div>})}{!data.length&&<p className="py-8 text-center text-sm text-white/45">Aún no hay datos para este indicador.</p>}</div>}
