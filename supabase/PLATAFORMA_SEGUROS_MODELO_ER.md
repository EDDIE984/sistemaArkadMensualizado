...
4. **Análisis de daños con IA (backend, disponible para avance parcial).** El
   usuario puede guardar las fotos disponibles y solicitar su análisis aun cuando
   falten tomas; la interfaz informa cuántas faltan y que la inspección sólo queda
   `COMPLETADA` al cubrirlas todas. El navegador procesa las fotos existentes en
   lotes. Por cada foto: se reduce a ~1024 px
...
16. **Inspección fotográfica (informativa).** `INSPECCION` es 1:0..1 con
    ...
    **Análisis de daños con IA:** el usuario puede solicitar el análisis de las
    fotos ya guardadas, incluso si `INSPECCION.estado='EN_PROGRESO'`. La interfaz
    mantiene visible el número de tomas faltantes; sólo al cubrirlas todas la
    inspección pasa a `COMPLETADA`. El navegador procesa por lotes (≤4 fotos por
    invocación para no chocar el timeout serverless). Por foto se
